...
def performance_segmentation(
    ...
    if plot:
        ...
        plot_segmentation(axes[2], beat_v, velocity_v[:-1], prob_, break_idx)
        plot_velocity_curve(axes[1], velocity_curve[0][:-1], break_idx)
        plot_beat_curve(axes[0], avg_beat_length[0], break_idx)
        plt.legend()
        fig.tight_layout()
